...
Vagrant.configure("2") do |config|
  ...
  if $is_windows then
  	  config.vm.box_url = "file://../xenial-server-cloudimg-amd64-vagrant.box"
	  config.vm.network "forwarded_port", guest: 80, host: 80
	  config.vm.network "forwarded_port", guest: 443, host: 443
  else
  	config.vbguest.auto_update = false #vagrant plugin install vagrant-vbguest
...
rdr pass inet proto tcp from any to any port 443 -> 127.0.0.1 port 10443
      ...
      system('echo "set packet filter 127.0.0.1:80/443 -> 127.0.0.1:10080/10443"')
    end
